...
export function getNewJoke() {
	return axios.get(hostname, requestHeaders).then(response => {
...
export function getSearchResults(searchText, currentPage) {
	...
	return axios.get(searchUrl, requestHeaders).then(response => response.data);
}

export function buildSearchUrl(searchText, currentPage) {
